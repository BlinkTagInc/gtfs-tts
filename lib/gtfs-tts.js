...
const gtfsTTS = async initialConfig => {
  ...
  if (!config.skipImport) {
    ...
    if (!config.gtfsPath && !config.gtfsUrl) {
      throw new Error('No gtfs info defined. Pass --gtfsPath or --gtfsUrl parameters or create a `config.json` file.');
    }
  ...
  config.log('For each stop, press "y" if pronunciation is correct or "n" if incorrect. Press "r" to repeat. Press "e" to end.\n');
  ...
  const readStopName = stop => {
    ...
    const answer = readlineSync.keyIn(`Stop ID ${stop.stop_id}: ${chalk.yellow(stop.stop_name)}\nPronunciation correct? (y/n/r/e): `);
